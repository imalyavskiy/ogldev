...
{
  vec4 AmbientColor = 
    vec4( Light.Color * Light.AmbientIntensity, 
          1.0 );
  float DiffuseFactor = 
  ...
  if (DiffuseFactor > 0.0) {
    DiffuseColor = 
      vec4( Light.Color * Light.DiffuseIntensity * DiffuseFactor, 
            1.0 );

    vec3 VertexToEye = 
      normalize(gEyeWorldPos - WorldPos);
    vec3 LightReflect = 
    ...
    if (SpecularFactor > 0.0) {
      ...
      SpecularColor = 
        vec4( Light.Color * gMatSpecularIntensity * SpecularFactor,
              1.0 );
    }
  }
...
vec4 CalcPointLight(vec3 WorldPos, vec3 Normal)
{
  ...
  vec4 Color = 
    CalcLightInternal( gPointLight.Base, 
                       LightDirection, 
                       WorldPos, 
                       Normal );
